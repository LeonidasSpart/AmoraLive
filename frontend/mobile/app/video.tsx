...
export default function Video(){
 ...
 return <AppShell><View style={s.page}><Pressable onPress={()=>router.back()}><Text style={s.back}>‹</Text></Pressable><View style={s.stage}><Text style={{fontSize:55}}>📹</Text><Text style={s.title}>{t("videoScreen.title")}</Text><Text style={s.name}>{peer?.display_name||peer?.username||t("videoScreen.memberFallback")}</Text><Text style={s.muted}>{t("videoScreen.readyLine")}</Text><Text style={s.muted}>{t("videoScreen.connectHint")}</Text></View><View style={s.actions}><Pressable style={s.end} onPress={()=>router.back()}><Text style={s.endText}>{t("videoScreen.endCall")}</Text></Pressable></View></View></AppShell>;
}

...
function normalizeCreators(creators:any[]){
  return (creators||[]).map((c:any)=>({key:`creator-${c.id}`,kind:"creator",id:c.id,title:c.display_name||c.username,image:c.profile_photo,isLive:false,meta:c.is_verified?"verified":"creator"}));
}

export default function Discover(){
 const {t}=useTranslation();
 const CATS_LABELS:Record<string,string>={"For You":t("discoverScreen.catForYou"),"Live":t("discoverScreen.catLive"),"Creators":t("discoverScreen.catCreators"),"Dating":t("discoverScreen.catDating"),"New":t("discoverScreen.catNew")};
 ...
 const load=useCallback(async()=>{
  ...
  }catch(e:any){setError(e.message||t("discoverScreen.errorLoad"));}
 ...
 return <AppShell><ScrollView style={s.page} contentContainerStyle={{paddingBottom:30}}>
  <Text style={s.kicker}>{t("discoverScreen.kicker")}</Text><Text style={s.title}>{t("discoverScreen.title")}</Text><Text style={s.sub}>{t("discoverScreen.subtitle")}</Text>
  <View style={s.search}><TextInput value={q} onChangeText={setQ} onSubmitEditing={()=>load()} placeholder={t("discoverScreen.searchPlaceholder")} placeholderTextColor={theme.dim} style={s.input}/><Pressable onPress={load} style={s.searchBtn}><Text>⌕</Text></Pressable></View>
  <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{marginVertical:12}}>{cats.map(c=><Pressable key={c} onPress={()=>setCategory(c)} style={[s.chip,category===c&&s.active]}><Text style={s.chipText}>{CATS_LABELS[c]||c}</Text></Pressable>)}</ScrollView>
  {loading?<ActivityIndicator color={theme.pink} style={{marginTop:30}}/>:error?<Text style={s.error}>{error}</Text>:<View style={s.grid}>{items.map((x,i)=><Pressable key={x.key||i} style={s.card} onPress={()=>x.kind==="room"?router.push({pathname:"/live/[id]",params:{id:String(x.id)}}):router.push({pathname:"/creator/[userId]",params:{userId:String(x.id)}})}>
    ...
    <Text numberOfLines={1} style={s.name}>{x.title||t("discoverScreen.memberFallback")}</Text>
    <Text numberOfLines={1} style={s.meta}>{x.isLive?t("discoverScreen.liveTag"):x.meta==="verified"?t("discoverScreen.verifiedCreator"):x.meta==="creator"?t("discoverScreen.creatorWord"):x.meta}</Text>
  </Pressable>)}</View>}
  {!loading&&!items.length&&<Text style={s.empty}>{t("discoverScreen.nothingHere")}</Text>}
 </ScrollView></AppShell>
}

...
export default function Rewards(){
 ...
 const load=useCallback(async()=>{try{const [s,h]=await Promise.all([api.dailyRewardStatus(),api.dailyRewardHistory()]);setStatus(s);setHistory(h||[]);}catch(e:any){setError(e.message||t("rewardsScreen.errorLoad"));}finally{setLoading(false);}},[]);
 useEffect(()=>{load();},[load]);
 const claim=async()=>{setClaiming(true);setError("");try{await api.claimDailyReward();await load();}catch(e:any){setError(e.message||t("rewardsScreen.errorClaimNotAvailable"));}finally{setClaiming(false);}};
 ...
 return <AppShell><ScrollView style={s.page} contentContainerStyle={{paddingBottom:40}}><Pressable onPress={()=>router.back()}><Text style={s.back}>‹</Text></Pressable><Text style={s.kicker}>{t("rewardsScreen.kicker")}</Text><Text style={s.title}>{t("rewardsScreen.title")}</Text><View style={s.hero}><Text style={s.emoji}>🎁</Text><Text style={s.streak}>{status?.currentStreak||0} {t("rewardsScreen.dayStreakSuffix")}</Text><Text style={s.muted}>{status?.nextReward?.coins||0} {t("rewardsScreen.coinsAvailableTodaySuffix")}</Text><Pressable disabled={claiming||claimed} onPress={claim} style={[s.claim,(claimed||claiming)&&s.disabled]}><Text style={s.claimText}>{claimed?t("rewardsScreen.claimedToday"):claiming?t("rewardsScreen.claiming"):t("rewardsScreen.claimDailyReward")}</Text></Pressable></View>{!!error&&<Text style={s.error}>{error}</Text>}<Text style={s.section}>{t("rewardsScreen.historyTitle")}</Text>{history.map((x,i)=><View key={x.id||i} style={s.row}><View style={{flex:1}}><Text style={s.day}>{x.claimed_at?new Date(x.claimed_at).toLocaleDateString():t("rewardsScreen.rewardFallback")}</Text><Text style={s.muted}>{t("rewardsScreen.dayOfCyclePrefix")} {x.day_number||1} {t("rewardsScreen.ofCycle")}{x.is_milestone?` · ${t("rewardsScreen.milestoneSuffix")}`:""}</Text></View><Text style={s.coins}>+{x.coins||0} 🪙</Text></View>)}</ScrollView></AppShell>;
}

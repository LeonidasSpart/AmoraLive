...
export default function Missions(){
 ...
 const load=useCallback(async()=>{try{setItems(await phase1Request("/missions"));}catch(e:any){setError(e.message||t("missionsScreen.errorLoad"));}finally{setLoading(false);}},[]);
 useEffect(()=>{load();},[load]);
 const claim=async(m:any)=>{setBusy(m.key);try{await phase1Request(`/missions/${m.key}/claim`,{method:"POST"});await load();}catch(e:any){setError(e.message||t("missionsScreen.errorClaim"));}finally{setBusy("");}};
 if(loading)return <AppShell><View style={s.center}><ActivityIndicator color={theme.pink}/></View></AppShell>;
 return <AppShell><ScrollView style={s.page} contentContainerStyle={{paddingBottom:40}}><Pressable onPress={()=>router.back()}><Text style={s.back}>‹</Text></Pressable><Text style={s.kicker}>{t("missionsScreen.kicker")}</Text><Text style={s.title}>{t("missionsScreen.title")}</Text>{!!error&&<Text style={s.error}>{error}</Text>}
  {["daily","weekly","lifetime"].map(type=>{const list=items.filter(x=>x.type===type);if(!list.length)return null;return <View key={type}><Text style={s.section}>{type==="daily"?t("missionsScreen.typeDaily"):type==="weekly"?t("missionsScreen.typeWeekly"):t("missionsScreen.typeLifetime")}</Text>{list.map(m=>{const pct=Math.min(100,Math.round((m.progress/m.target)*100));return <View key={m.key} style={[s.card,m.claimed&&{opacity:.6}]}><View style={s.top}><Text style={s.icon}>{m.icon}</Text><View style={{flex:1}}><Text style={s.name}>{m.title}</Text><Text style={s.muted}>{m.description}</Text></View></View><View style={s.track}><View style={[s.fill,{width:`${pct}%`}]}/></View><View style={s.metaRow}><Text style={s.muted}>{m.progress}/{m.target}</Text><Text style={s.reward}>🪙 {m.reward?.coins||0}</Text></View>{m.claimed?<Text style={s.claimed}>{t("missionsScreen.claimed")}</Text>:m.completed?<Pressable style={s.claim} disabled={busy===m.key} onPress={()=>claim(m)}><Text style={s.claimText}>{busy===m.key?t("missionsScreen.claiming"):t("missionsScreen.claimReward")}</Text></Pressable>:<Text style={s.inProgress}>{t("missionsScreen.inProgress")}</Text>}</View>})}</View>})}
 </ScrollView></AppShell>;
}

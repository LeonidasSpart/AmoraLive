...
export default function Settings(){
 ...
 const load=useCallback(async()=>{try{const u=await api.me();setUser(u);setDisplayName(u.display_name||"");setBio(u.bio||"");}catch(e:any){setError(e.message||"Unable to load settings.");}finally{setLoading(false);}},[]);
 useEffect(()=>{load();},[load]);
 const save=async()=>{setSaving(true);setError("");try{await api.updateProfile({display_name:displayName.trim(),bio:bio.trim()});Alert.alert("Saved","Your settings have been updated.");}catch(e:any){setError(e.message||"Unable to save settings.");}finally{setSaving(false);}};
 ...
 return <AppShell><ScrollView style={s.page} contentContainerStyle={{paddingBottom:40}}>
  ...
  <View style={s.card}><Text style={s.section}>Privacy</Text><Pressable style={s.row} onPress={()=>router.push("/security")}><Text style={s.rowText}>🛡️ Safety & Security</Text><Text style={s.arrow}>›</Text></Pressable></View>

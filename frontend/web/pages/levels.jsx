...
export default function Levels() {
  ...
  useEffect(() => { const t = localStorage.getItem('accessToken'); if (!t) return; fetch('https://api.amoramatch.one/users/me',{headers:{Authorization:`Bearer ${t}`}}).then(r=>r.ok?r.json():null).then(setUser).catch(()=>{}); }, []);
  ...
  return <Layout><div className="amora-level-page">
    ...
    <div className="amora-xp-card"><div className="amora-xp-top"><div><span>Current level</span><strong>Level {level}</strong></div><div><span>XP</span><strong>{xp}</strong></div></div><div className="amora-xp-track"><i style={{width:`${progress}%`}} /></div><small>{100-progress} XP until your next unlock</small></div>

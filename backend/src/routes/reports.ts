...
router.post('/:userId', authenticate, async (req: AuthRequest, res) => {
  ...
  if (!parsed.success) return res.status(400).json({ message: 'Invalid report', errors: parsed.error.issues });
  const report = await prisma.report.create({
    data: { reporterId: req.user.id, reportedId, reason: parsed.data.reason, details: parsed.data.details || null },
  });
